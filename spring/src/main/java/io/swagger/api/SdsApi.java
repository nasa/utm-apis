...
@Api(value = "sds", description = "the sds API")
public interface SdsApi {

    @ApiOperation(value = "Request information regarding SDSPs", notes = "Allows for querying for SDSPs.", response = Sds.class, tags={ "A. Discovery Endpoints", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful data request. Response includes requested data.", response = Sds.class),
        @ApiResponse(code = 400, message = "Bad request. Typically validation error. Fix your request and retry.", response = Sds.class),
        @ApiResponse(code = 429, message = "Too many recent requests from you. Wait to make further queries.", response = Sds.class) })
    @RequestMapping(value = "/sds",
        produces = { "application/json" }, 
        consumes = { "application/json" },
        method = RequestMethod.GET)
    ResponseEntity<Sds> getSds(@ApiParam(value = "The specific subset of fields to return. Comma separated list. Default will be all fields.") @RequestParam(value = "fields", required = false) String fields,
    ...
    @ApiOperation(value = "Submit information regarding USS Operations", notes = "Allows for submission of USS opreations to the service discovery system.", response = Void.class, tags={ "A. Discovery Endpoints", })
    @ApiResponses(value = { 
        @ApiResponse(code = 201, message = "Supplemental Data Service plan received.", response = Void.class),
        @ApiResponse(code = 400, message = "Bad request. Typically validation error. Fix your request and retry.", response = Void.class),
        @ApiResponse(code = 403, message = "Invalid ID supplied.  Fix authorization and retry.", response = Void.class),
        @ApiResponse(code = 429, message = "Too many recent requests from you. Wait to make further submissions.", response = Void.class) })
    @RequestMapping(value = "/sds",
        produces = { "application/json" }, 
        consumes = { "application/json" },
        method = RequestMethod.POST)
    ResponseEntity<Void> postSds(@ApiParam(value = "Description of the data service" ,required=true ) @RequestBody Sds sds);


    @ApiOperation(value = "Submit updates to an existing USS Operation", notes = "Allows for SDSP to update an previous plan submission.", response = Sds.class, tags={ "A. Discovery Endpoints", })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful data request. Response includes requested data.", response = Sds.class),
        @ApiResponse(code = 400, message = "Bad request. Typically validation error. Fix your request and retry.", response = Sds.class),
        @ApiResponse(code = 429, message = "Too many recent requests from you. Wait to make further queries.", response = Sds.class) })
